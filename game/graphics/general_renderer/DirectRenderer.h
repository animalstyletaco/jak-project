 ...
 */
class BaseDirectRenderer : public BaseBucketRenderer {
 public:
   ...
   */
  void setup_common_state(BaseSharedRenderState* render_state);
 ...
 protected:
  ...
  void handle_prim(u64 val, BaseSharedRenderState* render_state, ScopedProfilerNode& prof);
  void handle_prim_packed(const u8* data,
                          BaseSharedRenderState* render_state,
                          ScopedProfilerNode& prof);
  void handle_rgbaq(u64 val);
  void handle_xyzf2(u64 val, BaseSharedRenderState* render_state, ScopedProfilerNode& prof);
  ...
  bool m_prim_graphics_state_needs_graphics_update = true;
  bool m_test_state_needs_graphics_update = true;
  bool m_blend_state_needs_graphics_update = true;

  struct SpriteMode {
    bool do_first_draw = true;
  } m_sprite_mode;

  u64 vertex_buffer_max_verts = 0;
};
