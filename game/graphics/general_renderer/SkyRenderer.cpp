...
void BaseSkyBlendHandler::handle_sky_copies(DmaFollower& dma,
  ...
  if (!m_enabled) {
    ...
    if (render_state->use_sky_cpu) {
      m_gpu_stats = cpu_blender_do_sky_blends(dma, render_state, prof);

    } else {
      m_gpu_stats = gpu_blender_do_sky_blends(dma, render_state, prof);
    }
  }
}
...
void BaseSkyBlendHandler::render(DmaFollower& dma,
  ...
  if (dma.current_tag().qwc != 8) {
    ...
    tfrag_renderer_render(dma, render_state, tfrag_prof);
    return;
  }
...
void BaseSkyBlendHandler::draw_debug_window() {
  ImGui::Separator();
  ...
  if (ImGui::TreeNode("tfrag")) {
    tfrag_renderer_draw_debug_window();
    ImGui::TreePop();
  }
}

BaseSkyRenderer::BaseSkyRenderer(const std::string& name, int my_id)
    : BaseBucketRenderer(name, my_id) {}
...
void BaseSkyRenderer::render(DmaFollower& dma,
  ...
  direct_renderer_render_gif(setup_packet.data, setup_packet.size_bytes, render_state, prof);
