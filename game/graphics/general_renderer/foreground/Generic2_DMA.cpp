 ...
 */
bool BaseGeneric2::check_for_end_of_generic_data(DmaFollower& dma, u32 next_bucket) {
 ...
 */
bool BaseGeneric2::handle_bucket_setup_dma(DmaFollower& dma, u32 next_bucket) {
...
void BaseGeneric2::reset_buffers() {
...
u32 unpack_vtx_positions(BaseGeneric2::Vertex* vtx, const u8* data, int vtx_count) {
...
u32 unpack_vertex_colors(BaseGeneric2::Vertex* vtx, const u8* data, int vtx_count) {
  for (int i = 0; i < vtx_count; i++) {
...
u32 unpack_vtx_tcs(BaseGeneric2::Vertex* vtx, const u8* data, int vtx_count) {
  for (int i = 0; i < vtx_count; i++) {
    s16 s, t;
    memcpy(&s, data + (i * 4), 2);
...
void BaseGeneric2Jak2::process_dma(DmaFollower& dma, u32 next_bucket) {
  ...
  while (generic_dma::is_nop_zero(vif_transfer)) {
    vif_transfer = dma.read_and_advance();
  }

  while (!generic_dma::is_jak2_end(vif_transfer)) {
