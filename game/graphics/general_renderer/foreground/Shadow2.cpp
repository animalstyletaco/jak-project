...
BaseShadow2::BaseShadow2(const std::string& name, int my_id) : BaseBucketRenderer(name, my_id) {
...
void BaseShadow2::reset_buffers() {
...
void BaseShadow2::render(DmaFollower& dma,
  ...
  while (true) {
    ...
    if (vif1.kind == VifCode::Kind::UNPACK_V4_32) {
      ...
      fmt::print("unhandled transfer in BaseShadow2\n{} bytes\n{}\n{}\n", transfer.size_bytes,
...
void BaseShadow2::buffer_from_mscal2(const InputData& in) {
  // draw top caps.
...
void BaseShadow2::buffer_from_mscal4(const InputData& in) {
  add_wall_quads(in.wall_index_data, in.top_vertex_data, in.bottom_vertex_data);
}

void BaseShadow2::buffer_from_mscal6(const InputData& in) {
  // draw top caps.
...
BaseShadow2::ShadowVertex* BaseShadow2::alloc_verts(int n) {
...
u32* BaseShadow2::alloc_inds(int n, bool front) {
  if (!front) {
...
const u8* BaseShadow2::add_cap_tris(const u8* byte_data, const u8* vertex_data, bool flip) {
...
const u8* BaseShadow2::add_flippable_tris(const u8* byte_data, const u8* vertex_data, bool flip) {
  const int num_single_tris = *byte_data++;
  for (int i = 0; i < 3; i++) {
    int v = *byte_data++;
    ASSERT(v == 0);
