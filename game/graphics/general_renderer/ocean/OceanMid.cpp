...
void BaseOceanMidJak2::prepare_for_run(DmaFollower& dma, BaseSharedRenderState* render_state) {
  common_ocean_renderer_init_for_mid();
  // first is setting base and offset
  {
    auto base_offset_tag = dma.read_and_advance();
...
void BaseOceanMid::run_call0() {
  run_call0_vu2c();
}

void BaseOceanMid::xgkick(u16 addr) {
  common_ocean_renderer_kick_from_mid((const u8*)&m_vu_data[addr]);
}
