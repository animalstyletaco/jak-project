...
void BaseOceanNear::draw_debug_window() {}
...
void BaseOceanNear::render(DmaFollower& dma,
  ...
  // direct setup
  {
    common_ocean_renderer_init_for_near();
  ...
  common_ocean_renderer_flush_near(render_state, prof);
}

void BaseOceanNear::xgkick(u16 addr) {
  common_ocean_renderer_kick_from_near((const u8*)&m_vu_data[addr]);
}
