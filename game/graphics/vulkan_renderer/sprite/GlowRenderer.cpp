...
GlowVulkanRenderer::GlowVulkanRenderer(std::shared_ptr<GraphicsDeviceVulkan> device,
  ...
  // m_ogl.probe_fbo = std::make_unique<SwapChain>(m_device, VkExtent2D{m_ogl.probe_fbo_w,
  // m_ogl.probe_fbo_h});
  ...
  for (int i = 0; i < kDownsampleIterations; i++) {
    ...
    m_ogl.downsample_fbos[i].texture->createImageView(
        VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, 1);
...
void GlowVulkanRenderer::InitializeGlowProbeInputAttributes() {
  ...
  m_glow_probe_vulkan_inputs.attributeDescriptions.insert(
      m_glow_probe_vulkan_inputs.attributeDescriptions.end(), attributeDescriptions.begin(),
...
void GlowVulkanRenderer::SwitchToShader(ShaderId shaderId) {
  switch (shaderId) {
    case ShaderId::GLOW_PROBE: {
      ...
      m_pipeline_config_info.bindingDescriptions = m_glow_probe_vulkan_inputs.bindingDescriptions;
      m_pipeline_config_info.shaderStages = m_glow_probe_vulkan_inputs.shaderStages;
      break;
    }
    case ShaderId::GLOW_PROBE_READ: {
      ...
      m_pipeline_config_info.bindingDescriptions =
          m_glow_probe_read_vulkan_inputs.bindingDescriptions;
    ...
    case ShaderId::GLOW_DRAW: {
      ...
      m_pipeline_config_info.bindingDescriptions = m_glow_draw_vulkan_inputs.bindingDescriptions;
...
void GlowVulkanRenderer::downsample_chain(BaseSharedRenderState* render_state,
  ...
  for (int i = 0; i < kDownsampleIterations - 1; i++) {
    ...
    vkCmdDrawIndexed(m_command_buffer, num_sprites * 5, 1, 0, 0, 0);
  }
}
...
void GlowVulkanRenderer::draw_probes(BaseSharedRenderState* render_state,
  ...
  m_ogl.probe_fbo->beginRenderPass(m_command_buffer);
  ...
  m_pipeline_layout.updateGraphicsPipeline(m_command_buffer,
                                           m_pipeline_config_info);
  m_pipeline_layout.bind(m_command_buffer);

  // glDrawElements(GL_TRIANGLE_STRIP, idx_end - idx_start, GL_UNSIGNED_INT,
  //                (void*)(idx_start * sizeof(u32)));
  vkCmdDrawIndexed(m_command_buffer, idx_end - idx_start, 1, idx_start, 0, 0);
  vkCmdEndRenderPass(m_command_buffer);
 ...
 */
void GlowVulkanRenderer::draw_sprites(BaseSharedRenderState* render_state,
                                      ScopedProfilerNode& prof) {
  VkPipelineInputAssemblyStateCreateInfo& inputAssembly = m_pipeline_config_info.inputAssemblyInfo;
  inputAssembly.primitiveRestartEnable = VK_TRUE;
  ...
  auto& sampler_create_info =
      m_ogl.downsample_fbos[kDownsampleIterations - 1].sampler->GetSamplerCreateInfo();
  ...
  vkCmdPushConstants(m_command_buffer, m_pipeline_config_info.pipelineLayout,
                     VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(float), (void*)&m_debug.glow_boost);
  ...
  for (u32 i = 0; i < m_next_sprite; i++) {
    ...
    vkCmdDrawIndexed(m_command_buffer, 5, 1, record.idx, 0, 0);
  }
}
