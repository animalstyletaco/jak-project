...
uint32_t typelink_v3(Ptr<uint8_t> link, Ptr<uint8_t> data) {
  ...
  auto type_ptr = jak3::intern_type_from_c(sym_name, method_count);
...
uint32_t symlink_v3(Ptr<uint8_t> link, Ptr<uint8_t> data) {
  ...
  // intern
  auto sym = jak3::intern_from_c(sym_name);
 ...
 */
uint32_t link_control::jak3_work_v3() {
      ...
      if (seg_id == DEBUG_SEGMENT) {
        if (!DebugSegment) {
          ...
          if (ofh->code_infos[seg_id].size == 0) {
            ...
            jak3::ultimate_memcpy(Ptr<u8>(ofh->code_infos[seg_id].offset).c(), src.c(),
                                  ofh->code_infos[seg_id].size);
          }
        }
...
uint32_t link_control::jak3_work_v2() {
    ...
    if (*m_reloc_ptr == 0) {
      ...
      while (true) {
        ...
        if ((relocation & 0x80) == 0) {
          ...
          goalObj = jak3::intern_from_c(name).cast<u8>();
        } else {
          // type!
          ...
          goalObj = jak3::intern_type_from_c(name, nMethods).cast<u8>();
        }
...
void ultimate_memcpy(void* dst, void* src, uint32_t size) {
  ...
  if (!(u64(dst) & 0xf) && !(u64(src) & 0xf) && !(u64(size) & 0xf) && size > 0xfff) {
    if (!gfunc_774.offset) {
      ...
      auto sym = jak3::find_symbol_from_c("ultimate-memcpy");
      if (sym->value() == 0) {
