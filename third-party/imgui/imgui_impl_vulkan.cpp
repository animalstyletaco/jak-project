...
//  ImTextureID! See https://github.com/ocornut/imgui/pull/914 for discussions.
...
//  2021-10-15: Vulkan: Call vkCmdSetScissor() at the end of render a full-viewport to reduce
//  likehood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without calling
//  vkCmdSetScissor() explicitly every frame. 2021-06-29: Reorganized backend to pull data from a
//  single structure to facilitate usage with multiple-contexts (all g_XXXX access changed to
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data,
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != NULL) {
        ...
        if (sizeof(ImTextureID) < sizeof(ImU64)) {
          // We don't support texture switches if ImTextureID hasn't been redefined to be 64-bit. Do
          // a flaky check that other textures haven't been used.
  ...
  // state
  // - And you forgot to call vkCmdSetViewport() and vkCmdSetScissor() yourself to explicitely set
...
static void ImGui_ImplVulkan_CreateFontSampler(VkDevice device,
                                               const VkAllocationCallbacks* allocator) {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
  if (bd->FontSampler)
    return;
...
static void ImGui_ImplVulkan_CreateDescriptorSetLayout(VkDevice device,
                                                       const VkAllocationCallbacks* allocator) {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
...
static void ImGui_ImplVulkan_CreatePipelineLayout(VkDevice device,
                                                  const VkAllocationCallbacks* allocator) {
  ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device,
                                            const VkAllocationCallbacks* allocator,
